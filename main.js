...
function displaySampleText(splitText) {
	const sampleTextEl = document.getElementById('template');
...
function updateSampleText(currentIndex, splitText) {
	...
	let wordsBeforeCurrent = splitText.slice(0, currentIndex),
			before = '';
	for (let i = 0; i < wordsBeforeCurrent.length; i++)
		before += (wordsBeforeCurrent[i] + ' ');
	// Create string containing all words following the current word.
	let wordsAfterCurrent = splitText.slice(currentIndex+1, splitText.length),
			after = '';
	for (let i = 0; i < wordsAfterCurrent.length; i++)
		after += (wordsAfterCurrent[i] + ' ');
...
function outputScore() {
	...
	results.innerHTML = `<ul>
												<li>Speed: ${totalWordCount} words per minute</li>
												<li>Accuracy: ${percentAccuracy} percent</li>
											</ul>`
